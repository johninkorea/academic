...
# gif 만들기
data2=np.loadtxt("fit_log",dtype=float).T
data2=np.copy(data2[:450])
...
while z<len(data):
    if fittness[z]>0:
        ...
        # plt.legend()
        plt.savefig(file2, bbox_inches='tight', pad_inches=0.1, dpi=300, facecolor="white")
        plt.cla()
...
z=0
while z<8:
    files1.append(files1[-1])
    files2.append(files2[-1])
    z+=1

save_gif_PIL("img/1.gif", files1, fps=2, loop=0)
save_gif_PIL("img/2.gif", files2, fps=2, loop=0)
